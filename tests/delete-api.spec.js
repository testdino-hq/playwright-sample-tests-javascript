...
test.describe("DELETE User API", () => {
  ...
  test("DELETE: Remove user 1", { tag: "@api" }, async ({ request }) => {
    const res = await request.delete("https://dummyjson.com/users/1");
    console.log("STATUS:", res.status());
    expect(res.status()).toBe(200);
  });

  test("DELETE: Remove non-existing user returns 200", { tag: "@api" }, async ({ request }) => {
    const res = await request.delete("https://dummyjson.com/users/9999");
    console.log("STATUS:", res.status());
    expect(res.status()).toBe(200);
  });

  test("DELETE: Remove user twice", { tag: "@api" }, async ({ request }) => {
  ...
  test("DELETE: Validate body is returned", { tag: "@api" }, async ({ request }) => {
