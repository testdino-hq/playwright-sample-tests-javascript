...
test.describe("PUT / PATCH Update User API", () => {
  ...
  test("Update non-existing user still returns 200", { tag: "@api" }, async ({ request }) => {
  ...
  test("Login success (valid creds)", { tag: "@api" }, async ({ request }) => {
  ...
  test("Login returns expected username in response when success", { tag: "@api" }, async ({ request }) => {
  ...
  test("Login token length sanity check", { tag: "@api" }, async ({ request }) => {
    const payload = { username: "kminchelle", password: "0lelplR" };
    const res = await request.post("https://dummyjson.com/auth/login", { data: payload });
    const body = await res.json();
  ...
  test("Login headers present in response (server may add CORS headers)", { tag: "@api" }, async ({ request }) => {
    const payload = { username: "kminchelle", password: "0lelplR" };
    const res = await request.post("https://dummyjson.com/auth/login", { data: payload });
